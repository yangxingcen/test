 ...
 * $param 验证非空的字段名
 */
function check_data($data,$param=array()){
    if(is_array($data)){
        $count = count($data);
        if(!$count){
            ajaxReturn(0,"数据有误");
 ...
 * 更新键名
 * */
function update_key($arr){
    $arr_Array = array_reduce($arr,function(&$arr_Array,$v){
        $arr_Array[$v['id']] = $v;
        return $arr_Array;
    });
    return $arr_Array;
...
function generateTree($items){
    $tree = array();
    $items_new = array_reduce($items,function(&$items_new,$v){
        $items_new[$v['id']] = $v;
        return $items_new;
    });
    foreach($items_new as $it){
        if(isset($items_new[$it['pid']])){
            $items_new[$it['pid']]['son'][] = &$items_new[$it['id']];
        }else{
            $tree[] = &$items_new[$it['id']];
        }
    }
    return $tree;
}
 ...
 * $latitude    纬度
 * */
function get_address($longitude,$latitude){
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://apis.map.qq.com/ws/geocoder/v1/?location=".$latitude.",".$longitude."&key=AXQBZ-ODQ3U-KMLVO-2HSU5-AQTC3-RQF5V");
    ...
    $result=array(
        'nation'=>$ad_info->nation, //中国
        'province'=>$ad_info->province,//浙江省
        'city'=>$ad_info->city,         //杭州市
        'district'=>$ad_info->district,//西湖区
        'address'=>$res->address,       //浙江省杭州市西湖区文三路
        'address_info'=>$ad_info->province.$ad_info->city.$formatted_addresses->rough,       //浙江省杭州市西湖区华星时代广场(文三路北)
    );
    return $result;
}
...
/**20171103 wzz
 * 后台JSON返回*/
function ajaxReturn($status = "0", $info = "JSON返回", $url = "",$data=""){
 ...
 * 处理img的url
 */
function deal_img($newimg,$type=""){
    if($type=="1"){
        $newimg_arr =array_filter(explode(',',$newimg));
        foreach ($newimg_arr as $key=>$value){
            $newimg_arr[$key]=C('__OSSURL__').$value;
        }
        return $newimg_arr;
    }
    return $newimg?C('__OSSURL__').$newimg:'';
}

function order_status_fc($status=0)
{   #订单状态（0已取消,1:待付款,2:待成团,3:待发货,4:部分发货,5:待收货,6:待评价,7:已完成,8:申请退款中,9:退款完成,10:拒绝退款,11:申请售后, 12:退款关闭 13:前台删除）
    $statusarr = array('已取消','待付款','待成团','待发货','部分发货','待收货','待评价','已完成','申请退款中','退款完成','拒绝退款','申请售后','退款关闭','前台删除');
    return $statusarr[$status];
}
function pay_type_fc($type=0)
{   #支付方式：1-微信支付，2-支付宝，3-银联,4-余额
    if(!$type){
        $type = 0;
    }
    $typeArr = array('其他','微信','支付宝','银联','余额');
    return $typeArr[$type];
}
function filter_money($money,$accuracy=2)
{                                                                               #Cyonger iwanjiao@qq.com  201710011932
    return sprintf("%.2f", $money);
}
function shareCode($id=0)
{                                                                               #Cyonger  分享码
    $randstr  = 'aNbSc9dXeYf6gKhiVjkTlm0noWpFqr8stBu1vwEx5yzAC2DGH4IL7MZO3PQRJU';
    $sharecode=$randstr[rand(0,61)].$randstr[rand(0,61)].$randstr[rand(0,61)].$randstr[rand(0,61)].$randstr[rand(0,61)].$randstr[rand(0,61)].$id;
    return $sharecode;
}
function J($str){
...
function get_express_messageold($express_sign, $express_code){
    $ShipperCode = $express_sign;
    $LogisticCode = $express_code;
...
function encrypt_pass($pass){
    return md5(md5($pass).md5(C('PASS_KEY')));
}
...
function getpage($count, $pagesize = 10) {
    ...
    $p->setConfig('header', '<br/><li class="rows" style="margin-top:6px;">共<b>%TOTAL_ROW%</b>条记录&nbsp;第<b>%NOW_PAGE%</b>页/共<b>%TOTAL_PAGE%</b>页</li>');
    $p->setConfig('prev', '上一页');
    $p->setConfig('next', '下一页');
    $p->setConfig('last', '末页');
    $p->setConfig('first', '首页');
    $p->setConfig('theme', '%FIRST%%LINK_PAGE%%END%');
...
* return string
*/
 function qrcode($data,$filename,$picPath=false,$logo=false,$size='10',$level='H',$padding=3,$saveandprint=false){
 ...
 * 发送短信验证码
 * $telephone   手机号
 * $type        短信类型1-PC注册用户  2-商城注册用户  3-微信注册用户
 */
/*限制：一分钟之内只能发一条，一个手机号一天只最多发10条，*/
function sendMessage($telephone, $type)
{
    header("Content-Type: text/html; charset=utf8");
    if ($telephone == "" || $type == "") {
        return array("status" => 0, "info" => "缺少参数！");
    }
    $add_time_s = M('user_verify')->where(array('telephone' => $telephone))->getField("add_time");
    if ($add_time_s) {
        if (time() - strtotime($add_time_s) < 60) {
            return array("status" => 0, "info" => "1分钟后再发送");
        }
    }
    $times = array(
        '0' => strtotime(date("Y-m-d") . ' 00:00:00'),
        '1' => strtotime(date("Y-m-d") . ' 23:59:59'),
    );
    $count_s = M('user_verify')->where(array('telephone' => $telephone, 'add_time' => array('between', $times)))->count();
    if ($count_s > 10) {
        return array("status" => 0, "info" => "今天发送数量超过限制10条");
    }
    $sms_code = rand(100000, 999999);
    $templates = M("sms_template")->where(array('id' => $type))->find();
    if ($templates) {
//        if (in_array($type, array('1', '2', '3',))) { #验证码  $sms_code
            $msg = str_replace("\$sms_code", $sms_code, $templates['msg']);
//        }
    } else {
        return array("status" => 0, "info" => "短信内容不存在");
    }
    $sms = M('Sms')->find(1);
    /*发送短信 1*/
    $url ='http://api.ums86.com:8888/sms/Api/Send.do';
    $params = array(
        "SpCode"          => $sms['spcode'],
    ...
    $res = iconv('GB2312', 'UTF-8//IGNORE', httpClient($url,$data));
    $resArr = array();
    ...
    if (!empty($resArr) && $resArr["result"] == 0){
        ...
        M('user_verify')->add($data);
        return array("status" => 1, "info" => "手机短信验证码已发送");
    }else {
        return array("status" => 0, "info" => isset($resArr["description"]) ?$resArr["description"] :"手机短信验证码发送失败");
    }
    /*发送短信 2*/
 ...
 * curl post
 */
function httpClient($postUrl,$postData) {
    try {
        $ch = curl_init();
 ...
 * $sms_code        短信验证码
 * $type            短信验证码类型
 * status           验证码状态0-未过期 1-已验证  99-已过期
 */
function checkMessage($telephone, $sms_code, $type)
{
    $over_time = 120;
    $res = M('user_verify')->where(array('telephone' => $telephone, 'type'=>$type,"status" =>'0'))->order('id desc')->find();
    if (!$res) {
        return array("status" => '0', "info" => "手机短信验证码错误");
    }
    $dif_time = time() -strtotime($res['add_time']);
    if ($dif_time < $over_time) {
        $r_sms_code = $res['sms_code'];
        ...
        M('user_verify')->where(array('id' => $res['id']))->save(array("status" => 99));
        return array("status" => '0', "info" => "手机短信验证码已过期");
    }
}
 ...
 * 正则验证
 */
function preg_check_rule($type, $value){
    switch($type){
        case "phone":
            return preg_match("/^1[35789][0-9]{9}$/", $value);
            break;
        case "password":
            return true;
            return preg_match("/^(?![0-9]+$)[0-9A-Za-z]{6,10}$/", $value);
            break;
        default:
            return false;
            break;
...
// 获取评价列表
function discuss($type,$orderid){
    if($type == 1){
        $list = M("integral_order_info")->where("order_id='".$orderid."'")->select();
        foreach ($list as &$v) {
            $cate_id = M('integral_goods')->where("id='".$v['goods_id']."'")->getField("cate_id");
            $v['cate_name'] = M("integral_shop_type")->where("id='".$cate_id."'")->getField("name");
            $v['discuss_pic'] = explode(",", $v['discuss_pic']);
        }
        $res['shop'] = $list;
        $res['type'] = 1;
        $res['order_id'] = $orderid;
    }else{
        $res = M("order_goods")->where("order_id='".$orderid."'")->select();
        $res['type'] = 2;
    }
    return $res;
}
// 获取评价列表
function shop_discuss($type,$orderid){
    if($type == 1 || $type == 2){
        $model = M('goods');
    } else {
    ...
    $list = M("order_goods")->where("id='".$orderid."'")->field('id, order_id, goods_id, goods_nums, goods_pic as goods_logo, goods_name, goods_price as price')->select();
    foreach ($list as &$v) {
        $cate_id = $model->where("id='".$v['goods_id']."'")->getField("cate_id");
        $v['cate_name'] = M("shop_goods_cate")->where("id='".$cate_id."'")->getField("classname");
        $v['discuss_pic'] = explode(",", $v['discuss_pic']);
    }
    $res['shop'] = $list;
    $res['type'] = 1;
...
function browse($user_id){
    $info = M('browse')->where("user_id='".$user_id."' and is_del=0")->order("add_time desc")->select();
    foreach ($info as &$v) {
     ...
     * 领取优惠券
      ...
      * $userid，用户id，
     */
function getcoupon($couponid,$userid){
        if($couponid){
            $res = M('coupon')->where(['couponid'=>$couponid])->find();
            if(!$res){
                return array("status" => '0', "info" => "优惠券不存在");
            }
        }
        if($userid){
            $res1 = M('member')->where(['id'=>$userid])->find();
            if(!$res1){
                return array("status" => '0', "info" => "用户不存在");
            }
        }
        
        $query = M("coupon_data")->where("member_id = '".$userid."' and couponid='".$couponid."'")->count();
        if($query){
            return array("status" => '0', "info" => "优惠卷已领取");
        }
...
function applyTakeMoney($userid){
    ...
    if($money>$userinfo['wallet']){
        return array('status'=>0 ,"info"=>"您的账号余额不足！");
    }
    $bank = M("bank_list")->where(array('user_id'=>$userid,"id"=>$bankid))->find();
    //dump($bank);die;
    if(!$bank){
        return array('status'=>0 ,"info"=>"请选择正确的银行卡！");
    }
    unset($bank['id']);
    ...
    $m_m->startTrans();
    $t_m->startTrans();
    $res = $m_m->where(array('id'=>$userid))->setDec("wallet", $money);
    if($res){
        $res1 = $t_m->add($data);
        if($res1){
            $m_m->commit();
            $t_m->commit();
            return array('status'=>1 ,"info"=>"申请提现成功，请耐心等待！");
        }else{
            $m_m->rollback();
            $t_m->rollback();
            return array('status'=>0 ,"info"=>"申请提现失败！");
        }
    }else{
        $m_m->rollback();
        return array('status'=>0 ,"info"=>"申请提现失败！");
