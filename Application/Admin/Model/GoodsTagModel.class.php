...
{   //商品评价标签
    public function getTagList($storeid=1)
    {
        $where = array('storeid'=>$storeid,'isdel'=>0,'pid'=>0);
        $lists = $this->where($where)->order('sorts ASC,id DESC')->select();
        if($lists){
            foreach($lists AS $k=>$v){
                unset($v['create_at']);
                $ppp = array('storeid'=>$storeid,'isdel'=>0,'pid'=>$v['id']);
                $children = $this->where($ppp)->order('sorts ASC,id DESC')->select();
                if($children){
                    $allid = '';
    ...
    {   // 添加
        ...
        {   //添加修改
            case 'addEdit':
                ...
                $data = array('classname'=>$classname,'sorts'=>$sorts,'storeid'=>$storeid,'pid'=>$pid);
                if($id > 0){
                    if($pid){
                        $count = $this->where(array('pid'=>$id))->count();
                        if($count){
                            return array("status" => 0,"info" => '类目下有子集不可更改');
                        }
                    }
                    $retitle = '修改';
