...
use Think\Model;
class IngetralskuModel extends Model
{   
    ...
    {
        $table_str = "<tr></tr>";
        $goods_skus = M("integral_sku_list")->where(array("goods_id"=>$id, "status"=>1))->select();
        ...
        foreach($goods_skus as $k=>$v){
            ...
            $table_str .= "<td >
                    ...
                    <td><input name='store' value='{$v['store']}' class='input'></td>
                    <td><input name='price' value='{$v['price']}' class='input'></td>
                    ";
            $table_str .= "</tr>";
        }
        return $table_str;
